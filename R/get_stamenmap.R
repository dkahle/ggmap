...
get_stamenmap <- function(
  ...
  # Remove in 2.5
  if (!missing(crop)) {
    warning("crop argument is deprecated", call. = FALSE)
  }
  if (!missing(urlonly)) {
    warning("urlonly argument is deprecated", call. = FALSE)
  }
  if (!missing(color)) {
    warning("color argument is deprecated", call. = FALSE)
  }

  fetch_region(bbox[c("left", "right")], bbox[c("bottom", "top")], zoom = zoom,
    provider = "stamen", style = maptype, cache = !force)

}

check_bbox <- function(x) {
  if (!is.numeric(x) || length(x) != 4) {
    stop("bounding box improperly specified.", call. = FALSE)
  }
  if (is.null(names(x))) {
...
check_zoom <- function(x) {
  if (!is.numeric(x) || length(x) != 1) {
    stop("zoom must be a numeric vector of length 1", call. = FALSE)
  }

  x <- round(x)

  if (x < 0 | x > 18) {
    stop("zoomm must lie in 0-18", call. = FALSE)
  }
