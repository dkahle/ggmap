#' Get a Naver Map
#' 
#' get_navermap accesses the Naver Static Maps API version 1.1 to download a 
#' static map.  Note that in most cases by using this function you are agreeing 
...
get_navermap <- function(
  ...
  markers, filename='',messaging = FALSE, urlonly = FALSE,force = FALSE, ...
){
  ...
  if('level' %in% argsgiven){
    if(!(is.numeric(level) && level == round(level) && level > 0)){
      stop('level must be a whole number between 1 and 14', call. = F)
    }  
  }
  ...
  # format url proper
  post_url <- paste(center_url,crs_url ,level_url, size_url, baselayer_url, overlayers_url,
  ...
  ll <- getBorderLonLat(center[1], center[2], level, size[1], size[2])
